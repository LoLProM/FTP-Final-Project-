...
def handle_pass(ftp_server, client_socket, args):
    """
    ...
    password = args[0]
    if ftp_server.current_user:
        if password == ftp_server.users[ftp_server.current_user]:
            ftp_server.authenticated = True  # Cliente autenticado
...
def handle_pwd(ftp_server, client_socket, args):
    """
    ...
    response = f"257 \"{ftp_server.current_dir.relative_to(ftp_server.base_dir)}\"\r\n"
    client_socket.send(response.encode())

def handle_cwd(ftp_server, client_socket, args):
    """
    ...
    if not args or len(args) > 1:
        ...
        return
    try:
        new_path = (ftp_server.current_dir / args[0]).resolve()
        if new_path.exists() and new_path.is_dir():
            ftp_server.current_dir = new_path
...
def handle_cdup(ftp_server, client_socket, args):
    """
    ...
    if args:
        ...
        return
    try:
        if ftp_server.current_dir == ftp_server.base_dir:
            client_socket.send(b"550 Cannot go up. You are already in the root directory.\r\n")
        else:
            new_path = ftp_server.current_dir.parent.resolve()
            if new_path.exists() and new_path.is_dir():
                ftp_server.current_dir = new_path
                client_socket.send(b"250 Directory successfully changed\r\n")
            else:
...
def handle_list(ftp_server, client_socket, args):
    """
    ...
    try:
        path = ftp_server.current_dir / args[0] if args else ftp_server.current_dir
...
def handle_mkd(ftp_server, client_socket, args):
    """
    ...
    if not args or len(args) > 1:
        ...
        return
    try:
        new_dir = (ftp_server.current_dir / args[0])
...
def handle_rmd(ftp_server, client_socket, args):
    """
    ...
    if not args or len(args) > 1:
        ...
        return
    try:
        dir_to_remove = (ftp_server.current_dir / args[0])
...
def handle_dele(ftp_server, client_socket, args):
    """
    ...
    if not args or len(args) > 1:
        ...
        return
    try:
        file_to_delete = (ftp_server.current_dir / args[0])
...
def handle_rnfr(ftp_server, client_socket, args):
    """
    ...
    if not args or len(args) > 1:
        client_socket.send(b"501 Syntax error in parameters or arguments\r\n")
        return
    file_path = (ftp_server.current_dir / args[0])
    if file_path.exists():
        ftp_server.rename_from = file_path
        client_socket.send(b"350 Ready for RNTO\r\n")
    else:
        client_socket.send(b"550 File not found\r\n")

def handle_rnto(ftp_server, client_socket, args):
    """
    ...
    try:
        ...
        ftp_server.rename_from.rename(new_path)
        client_socket.send(b"250 File renamed successfully\r\n")
    except:
        client_socket.send(b"553 Error renaming file\r\n")
    finally:
        ftp_server.rename_from = None

def handle_syst(ftp_server, client_socket, args):
    """
...
def handle_help(ftp_server, client_socket, args):
    """
...
def handle_noop(ftp_server, client_socket, args):
    """
...
def handle_quit(ftp_server, client_socket, args):
    """
...
def handle_acct(ftp_server, client_socket, args):
    """
...
def handle_smnt(ftp_server, client_socket, args):
    """
...
def handle_rein(ftp_server, client_socket, args):
    """
    ...
    if args:
        client_socket.send(b"501: Syntax error in parameters or arguments.\r\n")
        return
    ftp_server.current_user = None
    ftp_server.authenticated = False
    ftp_server.current_dir = ftp_server.base_dir
...
def handle_pasv(ftp_server, client_socket, args):
    ...
    try:
        ...
        ftp_server.pasv_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ftp_server.pasv_socket.bind((ftp_server.host, 0))
        ftp_server.pasv_socket.listen(1)
        _, port = ftp_server.pasv_socket.getsockname()
...
def handle_abor(ftp_server, client_socket, args):
    client_socket.send(b"226 ABOR procesado\r\n")

def handle_site(ftp_server, client_socket, args):
    client_socket.send(b"200 Comando SITE no soportado\r\n")

def handle_stat(ftp_server, client_socket, args):
    response = "211-Estado del servidor FTP\r\n"
    response += f"    Usuario: {ftp_server.current_user}\r\n"
    response += f"    Directorio actual: {ftp_server.current_dir}\r\n"
